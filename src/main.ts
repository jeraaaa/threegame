...
function world() {
    ...
    box.initGraphics(new THREE.Mesh(new THREE.BoxGeometry(2, 2, 2), new THREE.MeshPhongMaterial({ color: 0xff0000 })), new THREE.Vector3(0, 5, 0));
    box.initPhysics(new Ammo.btBoxShape(new Ammo.btVector3(1, 1, 1)), 1);
}
world();
...
function loop() {
    ...
    Engine.camera.position.set(playerPos.x(), playerPos.y() + 1, playerPos.z());
